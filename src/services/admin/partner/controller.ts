...
export const createPartner = async (req: any, res: any) => {
  try {
    const { email, laundryName, name, phoneNumber, openingTime, closingTime, street, city, state, address, postalCode, location } = req.body;

    const checkPartnersInDb = await partnerModel.find();
    // if (checkPartnersInDb?.length >= 1) {
    //   throw new HTTP400Error(
    //     Utilities.sendResponsData({
    //       code: 400,
    //       message: "One partner already exists."
    //     })
    //   )
    // }
    ...
    const randomPassword: any = await Utilities.generateRandomPassword(8);
    ...
    const savedPartner: any = await newPartner.save();

    let messageHtml = await ejs.renderFile(
      process.cwd() + "/src/views/ragisterPartner.ejs",
      {
...
export const getPartnerList = async (token: any, queryParams: any, res: any) => {
  try {
    ...
    // Count documents once
    const totalCount = await partnerModel.countDocuments(query);
    const totalPages = Math.ceil(totalCount / limit);

    // Build the query
    let partnerQuery = partnerModel.find(query).sort({ createdAt: -1 });

    // if (applyPagination) {
    //   partnerQuery = partnerQuery.skip(skip).limit(limit);
    // }

    const partnerRes = await partnerQuery.exec();
...
export const getPartnerDetail = async (id: string, res: any) => {
  try {
    let partnerRes = await partnerModel.findOne({ _id: mongoose.Types.ObjectId(id), isDeleted: false });
...
export const deletePartner = async (id: string, res: any) => {
  try {
    ...
    if (partnerRes) {
      partnerRes.isDeleted = true;
      let result = await partnerRes.save();
